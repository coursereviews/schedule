window.onload = function displayResults(){
  ...
  var Click = function(clicked){
    ...
    if(clicked.className !== 'list-group-item clicked'){clicked.className = 'list-group-item clicked';}
    else{clicked.className = 'list-group-item';}

    if(clicked.attributes[1].value === "department"){selDept = unclick(selDept, clicked);}
    else if(clicked.attributes[1].value === "requirement"){selReq = unclick(selReq, clicked);}
    else if(clicked.attributes[1].value === "location"){selLoc = unclick(selLoc, clicked);}

    function unclick(selected, newItem){
      if(selected.length>0){selected[0].className = 'list-group-item';}
