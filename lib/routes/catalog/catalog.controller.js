  ...
  .then(function(results) {
    ...
    results.models.forEach(function(model, i) {
      ...
      .then(function(course) {
        ...
        course.related('courseOfferings').fetch({withRelated:['term','professors','schedules','requirements','meetings']})
...
exports.departmentQuery = function(req, res) {
  ...
  .then(function(results) {
    ...
    var len = results.models.length;
    results.models.forEach(function(model, i) {
      model.fetch({withRelated: ['courses', 'courses.courseOfferings', 'courses.courseOfferings.term', 'courses.courseOfferings.professors', 'courses.courseOfferings.requirements', 'courses.courseOfferings.meetings']})
      .then(function(dept) {
...
exports.professorQuery = function(req, res) {
  ...
  queryModel.query(function(qb){
    ...
    queryKey.forEach(function(key){
      //if the value is a string
      if(typeof queryObj[key] === 'string') {
        qb.where(key,'LIKE',"%"+queryObj[key]+"%"); //this one returns if the key is a part of another word
      }
      //if the value is an array
      else {
        for (var index=0;index<queryObj[key].length;index++) {
          qb.where(key,'LIKE',"%"+queryObj[key][index]+"%"); //this one returns if the key is a part of another word
        }
      }
    });
  ...
  .then(function(results) {
    ...
    results.models.forEach(function(model, i) {
      ...
      .then(function(prof) {
        ...
        prof.related('courseOfferings').fetch({withRelated:['term','course','schedules','requirements','meetings']})
        .then(function(offering) {
  ...
  .then(function(results) {
    ...
    results.models.forEach(function(model, i) {
      ...
      .then(function(requirement) {
        ...
        requirement.related('courseOfferings').fetch({withRelated:['term','course','schedules','professors','meetings']})
        .then(function(offering) {
