...
// can just iterate over the keys and values and put in where instructions
  ...
  queryModel.query(function(qb) {
    ...
    queryKey.forEach(function(key) {
      ...
      if (typeof queryObj[key] === 'string') {
        if (key === 'id') {
          qb.where(key, '=', queryObj[key]);
        } else { qb.where(key, 'LIKE', '%' + queryObj[key] + '%'); }
      }
  ...
  .then(function(results) {
    ...
    results.models.forEach(function(model, i) {
      ...
      .then(function(course) {
        ...
        course.serializeWithFavorited({user: userData, omitPivot: true})
            .then(function(results){
              var temp = results;
              course.related('courseOfferings')
                  ...
                  .then(function(offering) {
                    ...
                    model.serializeWithFavorited({user: userData, omitPivot: true});
    ...
    .then(function(results) {
      ...
      results.models.forEach(function(model, i) {
        ...
        .fetch({withRelated: ['courses',
                              ...
                              'courses.courseOfferings.professors',
                              'courses.courseOfferings.requirements',
                              'courses.courseOfferings.meetings']})
            .then(function(dept) {
               Promise.map(dept.related('courses').models, function(model) {
                return model.serializeWithFavorited({user: user, omitPivot: true});
                }).then(function(stuff) {
              //set the course with favorite to the json
              var deptJSON = dept.toJSON();
              deptJSON['courses'] = stuff;
              resultList.push(deptJSON);
              if (i === len - 1) {
                res.send(resultList);
                return resultList;
              }
...
exports.meetingQuery = function(req, res) {
  ...
  .then(function(results) {
    ...
    var len = results.models.length;
    results.models.forEach(function(model, i) {
      model.fetch({withRelated: ['courseOffering',
                                 'courseOffering.requirements',
    ...
    results.models.forEach(function(model, i) {
      ...
      .then(function(prof) {
        ...
        .fetch({withRelated:['term', 'course', 'schedules',
                             'requirements', 'meetings']})
        .then(function(offering) {
          //console.log(offering.models[0].related('course'));
          //offering.models.forEach(function(offers){
          //  offers.related('course')
          //      .then(function(cour){
          //        return cour.serializeWithFavorited({user: user, omitPivot: true});
          //
          //      }).then(function(result){
          //    courseArray.push(result);
          //    console.log("cArray:", result);
          //  });
          //
          //});
          temp.courseOfferings = offering.toJSON();
