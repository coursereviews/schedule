...
// can just iterate over the keys and values and put in where instructions
  ...
  queryModel.query(function(qb){
    ...
    //   if(typeof queryObj[queryKey[0]] === 'string') {
    //     qb.where(queryKey[0], 'LIKE', "%"+queryObj[queryKey[0]]+"%");
    //   }
    //   else {
    //     // qb.where(queryKey[0], 'LIKE', "%"+queryObj[queryKey[0]][0]+"%");
    //     for (var index=0;index<queryObj[queryKey[0]].length;index++) {
    //       qb.where(queryKey[0],'LIKE',"%"+queryObj[queryKey[0]][index]+"%");
    //     }
    //   }
    //   for (var keyIndex=1;keyIndex<queryKey.length;keyIndex++) {
    //     if(typeof queryObj[queryKey[keyIndex]] === 'string') {
    //       qb.orWhere(queryKey[keyIndex], 'LIKE', "%"+queryObj[queryKey[keyIndex]]+"%");
    //     }
    //     else {
    //       // qb.where(queryKey[0], 'LIKE', "%"+queryObj[queryKey[0]][0]+"%");
    //       for (var index=0;index<queryObj[queryKey[keyIndex]].length;index++) {
    //         qb.orWhere(queryKey[keyIndex],'LIKE',"%"+queryObj[queryKey[keyIndex]][index]+"%");
    //       }
    //     }
    //   }
    // }
