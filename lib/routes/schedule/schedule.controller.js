...
exports.updateSchedule = function(req, res) {
  ...
  .spread(function(schedule, user) {
    ...
    const toInt = function(i) { return +i; };

    const currentCourseOfferings = schedule.related('courseOfferings').pluck('id');
    const patchCourseOfferings = (req.body.courseOfferings || []).map(toInt);
    const currentExtraCurriculars = schedule.related('extraCurriculars').pluck('id');
    const patchExtraCurriculars = (req.body.extraCurriculars || []).map(toInt);
    ...
    return Promise.all(scheduleSetOps);
  })
...
exports.deleteSchedule = function(req, res) {
  Promise.all([
    new Schedule({id: req.params.id}).fetch({withRelated: ['user', 'courseOfferings', 'extraCurriculars']}),
    req.user.fetch()
  ])
  .spread(function(schedule, user) {
    if (!schedule || schedule.related('user').get('id') !== user.get('id')) {
